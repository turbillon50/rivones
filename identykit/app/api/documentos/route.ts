...
export async function GET() {
  ...
  try {
    ...
    const docs: Row[] = await sql`SELECT tipo, archivo_nombre, created_at FROM identykit.documentos WHERE user_id=${uid}`;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const existing: Row[] = await sql`SELECT id FROM identykit.documentos WHERE user_id=${uid} AND tipo=${tipo} LIMIT 1`;
    if (existing.length) {
